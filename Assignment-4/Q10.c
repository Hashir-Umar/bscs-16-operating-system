...
typedef struct{
  ...
  ListNode_t* tail;
  int sizeValue;
  pthread_mutex_t*  size_mutex; //using size mutex to ensure that multiple threads of insert/delete wont increment/decrement sizeValue
...
DoubleList_t* makeList(){
  ...
  ls->size_mutex = malloc(sizeof(pthread_mutex_t));

  ls->sizeValue = 0;
  pthread_mutex_init(ls->size_mutex, NULL);
...
void insert(DoubleList_t* ls, void* data){
  ...
  while(1){
    ...
    if(current != NULL){
      /* normal insertion somewhere in the middle of the list */
      current->prev = val;
      pthread_mutex_lock(ls->size_mutex);
      ls->sizeValue++;
      pthread_mutex_unlock(ls->size_mutex);
    }else{
